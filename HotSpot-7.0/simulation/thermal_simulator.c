...
//-------------------------------------------------------------------------------------------------------------------------
double ***max_grid_temp = NULL;
int grid_rows = 0, grid_cols = 0, grid_layers = 0, k;
//-------------------------------------------------------------------------------------------------------------------------
...
{
    // Return the stored dimension, or 0 if grid model wasn't used
    return (model && model->type == GRID_MODEL) ? grid_layers : 0;
}

int get_grid_row_count(void)
{
    return (model && model->type == GRID_MODEL) ? grid_rows : 0;
}

int get_grid_col_count(void)
{
    return (model && model->type == GRID_MODEL) ? grid_cols : 0;
}

double get_max_grid_temperature(int layer, int row, int col)
{
    // Safety checks before accessing the array
    if (max_grid_temp && layer >= 0 && layer < grid_layers && row >= 0 && row < grid_rows && col >= 0 && col < grid_cols)
    {
        return max_grid_temp[layer][row][col];
    }
    // Return 0.0 if out of bounds or if the model isn't a grid model
    return 0.0;
}
// ====================================================================================================================
#if VERBOSE > 1
...
void simulation(int *act)
{
    ...
    // if (act == NULL)
    // {
    //     fatal("unable to allocate memory for 'act' array\n");
    // }
    ...
    while ((num = read_vals(pin, vals)) != 0)
    {
                ...
                if (model->grid->layers[i].has_power)
                {
                    for (j = 0; j < model->grid->layers[i].flp->n_units; j++)
                    {
                        ...
                        if (act[count + j])
                            ...
                            power[base + idx] = 0; //, printf("%f", power[base + idx])
                    }
        ...
        lines++;
        // ==============================================================
        // If using the grid model, update the max temperatures
        if (model->type == GRID_MODEL && model->grid->last_trans)
        {
            for (k = 0; k < grid_layers; k++)
            {
                for (i = 0; i < grid_rows; i++)
                {
                    ...
                    {
                        max_grid_temp[k][i][j] = fmax(max_grid_temp[k][i][j], model->grid->last_trans->cuboid[k][i][j]);
                    }
                }
            }
        }
...
void initiation(int argc, char **argv)
{
    microchannel_config = malloc(sizeof(microchannel_config_t));
    ...
    else if (model->type == GRID_MODEL)
    {
        ...
        // =====================================================================
        // Get dimensions
        grid_layers = model->grid->n_layers;
        grid_rows = model->grid->rows;
        grid_cols = model->grid->cols;

        // Allocate memory for the 3D array
        max_grid_temp = (double ***)calloc(grid_layers, sizeof(double **));
        assert(max_grid_temp);
        for (k = 0; k < grid_layers; k++)
        {
            max_grid_temp[k] = (double **)calloc(grid_rows, sizeof(double *));
            assert(max_grid_temp[k]);
            for (i = 0; i < grid_rows; i++)
            {
                max_grid_temp[k][i] = (double *)calloc(grid_cols, sizeof(double));
                assert(max_grid_temp[k][i]);
            }
        }
        // =====================================================================
    }
    ...
    // xdim = model->grid->rows;
    // ydim = model->grid->cols;
    // layers = model->grid->n_layers;
}
...
void stop()
{
    ...
    if (max_grid_temp)
    {
        for (k = 0; k < grid_layers; k++)
        {
            for (i = 0; i < grid_rows; i++)
            {
                free(max_grid_temp[k][i]);
            }
            free(max_grid_temp[k]);
        }
        free(max_grid_temp);
        max_grid_temp = NULL;
    }
