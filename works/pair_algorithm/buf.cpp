...
void Thermal_block()
{
    ...
    for (int i = 0; i < Gh; i++)
    {
        for (int j = 0; j < Gw; j++)
        {
            for (int k = 0; k < Gl; k++)
            {
                mesh[i][j][k].temp = get_max_grid_temperature(i * 2, j, k);
                if (mesh[i][j][k].temp >= TEMP_THRESHOLD)
                    mesh[i][j][k].isnotBlocked = 0;
                else
                    mesh[i][j][k].isnotBlocked = 1;
            }
        }
    }
}
