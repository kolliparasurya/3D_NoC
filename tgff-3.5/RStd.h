...
#define Rassert(x) rstd::rassert((x), __FILE__, __LINE__)
	...
	inline void rconfirm(bool x, const char *message)
	{
		if (!x)
			rexit(message);
	}
...
#define Rdumphex(x) (rstd::cerrhex << #x << ": 0x" << (x) << std::endl)
	...
	{
		// No need for explicit static_assert() inside a valid specialization
	};
// No false definition.  Compile-time failure if assertion fails.
#define STATIC_ASSERT(x) rstd::my_static_assert<(x)>()
	...
	struct same_or_derived
	{
		enum
		{
			result = conversion_exists<const D *, const B *>::result &&
