...
#include "Hotspot_temperature_grid.h"
...
double blist_avg(blist_t *ptr, flp_t *flp, double *v, int type)
{
	...
	for(; ptr; ptr = ptr->next) {
		if (type == V_POWER){
			...
			//printf("%.2lf %.2lf %d %.2lf %.2lf track\n", ptr->occupancy, v[ptr->idx], ptr->idx, flp->units[ptr->idx].width, flp->units[ptr->idx].height);//surya
				}
		else if (type == V_TEMP)		   
			...
			for(j=j1; j < j2; j++)
				...
				if ((i > i1) && (i < i2-1) && (j > j1) && (j < j2-1)) {
					 ...
					 */
						...
						//printf("b2gmap[i][j] %d    i %d  j %d", layer->b2gmap[i][j],i,j);
					}
...
void populate_R_model_grid(grid_model_t *model, flp_t *flp)
{
	...
	if (model->width > model->config.s_sink || 
		...
		if(model->width > model->config.s_sink ||
			model->height > model->config.s_sink)
			fatal("inordinate floorplan size!\ndebug --> Hotspot_temperature.cpp: s_sink overflow!\n\n");
		if (model->width > model->config.s_spreader ||
			model->height > model->config.s_spreader)
			fatal("inordinate floorplan size!\ndebug --> Hotspot_temperature.cpp: s_spreader overflow!\n\n");
	}
	...
	for(n=0; n < model->n_layers; n++) {
		for(i=0; i < model->rows; i++)
			for(j=0; j < model->cols; j++) {
				...
				if (type == V_POWER){
					//printf("%.2lf, gcuboid  %d %d %d", g->cuboid[n][i][j], n, j, i);//surya
					g->cuboid[n][i][j] = blist_avg(model->layers[n].b2gmap[i][j], 
										 model->layers[n].flp, &b[base], type) * area;
					//if(n==1) printf("%.2lf track\n", b[base]);
// if(n==0) printf("%.2lf %.2lf %.2lf %.2lf track\n", model->layers[n].b2gmap[i][j]->occupancy, b[base], model->layers[n].flp->units[model->layers[n].b2gmap[i][j]->idx].width, model->layers[n].flp->units[model->layers[n].b2gmap[i][j]->idx].height);//surya
					// if(n==0)printf("%.2lf, gcuboid\n", g->cuboid[n][i][j]);//surya
					}
		 ...
		 * block power vector
		 */
		//printf("%.2lf, newgcuboid %d\n", g->cuboid[n][0][0], n);//surya
		// printf("%.2lf track\n", b[base]);//surya
		base += model->layers[n].flp->n_units;							 
	}
						...
						grid_model_vector_t *temp)
{
	...
	/* last layer	*/
		for(i=0; i < nr; i++)
			...
			// printf("%.2lf, %.2lf, %.2lf, 3parts\n", model->config.ambient, sum[i][j], model->layers[nl-1].rz);//surya
			// printf("%.2lf, temp->cuboid\n", temp->cuboid[nl-1][i][j]);//surya
			}
...
void recursive_multigrid(grid_model_t *model, grid_model_vector_t *power,
						 grid_model_vector_t *temp)
{
	// printf("%.2lf, model->last_steady->cuboid1\n", temp->cuboid[0][0][0]);//surya
	double delta;
	#if VERBOSE > 1
	unsigned int i = 0;
	#endif
	...
	if (model->rows <= 1 || model->cols <= 1) {
		...
		// printf("%.2lf, model->last_steady->cuboid2\n", temp->cuboid[0][0][0]);//surya
	...
	#endif
	// printf("%.2lf, model->last_steady->cuboid5\n", temp->cuboid[0][0][0]);//surya
}
...
void steady_state_temp_grid(grid_model_t *model, double *power, double *temp)
{
	...
	// printf("%.2lf , power-cuboid1\n",power[3]);//surya
	// printf("%.2lf , power-cuboid\n",p->cuboid[0][0][0]);//surya
	...
	// printf("%.2lf, temp\n", model->last_steady->cuboid[0][0][0]);//surya
