...
void Thermal_IF::hs_init()
{
	...
	cout << endl;
	// cout << hs_model->config->init_temp << " temperature" <<endl;//surya
...
void Thermal_IF::hs_steady( NoximTile *t[MAX_STATIC_DIM][MAX_STATIC_DIM][MAX_STATIC_DIM] )
{
	...
	char * cstr;
	// printf("%d, %.2lf, %d forgetzero\n", getCurrentCycleNum(), TEMP_REPORT_PERIOD,TEMP_REPORT_NUM);
	...
	cout << endl;
	// cout << hs_model->grid->n_layers << " view2" <<endl; //surya
					...
					for( m = 0 ; m < mesh_dim_x ; m++ ){
						...
						if( n != -1 ){
							...
							/*if( (hs_steady_temp[hs_base + hs_idx ] - 273.15) > 98 )
								...
								results_log_router<<"."<<"\t";
							*/
							//results_log_router << hs_steady_temp[hs_base + hs_idx ] <<endl; //surya
