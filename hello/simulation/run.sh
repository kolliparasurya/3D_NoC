...
# ../hotspot -c hotspot.config -p new_core3D.ptrace -grid_layer_file NoC_layer.lcf -materials_file example.materials -model_type grid -detailed_3D on -o gcc.ttrace -grid_transient_file gcc.grid.ttrace
...
../hotspot -c hotspot.config -f new_core_layer0.flp -p new_core3D.ptrace -steady_file gcc.steady -model_type grid -detailed_3D on -grid_layer_file NoC_layer.lcf -grid_steady_file gcc.grid.steady
cp gcc.steady gcc.init
# Transient simulation with steady-state results as initial temperature:
../hotspot -c hotspot.config -init_file gcc.init -f new_core_layer0.flp -p new_core3D.ptrace -o gcc.ttrace -model_type grid -detailed_3D on -grid_layer_file NoC_layer.lcf -grid_transient_file gcc.grid.ttrace
